...
.PHONY: deps
deps: info install-policy install-svu
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"

.PHONY: build
build:
	@echo -e "$(ATTN_COLOR)==> $@ $(NO_COLOR)"
	@${EXT_BIN_DIR}/policy build src --tag ghcr.io/gertd/test-policy:latest

.PHONY: tag
tag:
	@echo -e "$(ATTN_COLOR)==> $@ $(svu patch) $(NO_COLOR)"
	@${EXT_BIN_DIR}/policy tag ghcr.io/gertd/test-policy:latest ghcr.io/gertd/test-policy:$$(svu)

.PHONY: push
push:
	@echo -e "$(ATTN_COLOR)==> $@ $(svu patch) $(NO_COLOR)"
	@${EXT_BIN_DIR}/policy push ghcr.io/gertd/test-policy:$$(svu)
